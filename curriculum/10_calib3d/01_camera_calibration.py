...
# =============================================================================
# 2. LOAD OR CREATE CHECKERBOARD
# =============================================================================
print("\n--- 2. Loading/Creating Test Pattern ---")
...
def load_checkerboard_image():
    """Load a real checkerboard image or create synthetic one."""
    # Try to load real checkerboard images
    for sample in ["chessboard.png", "checkerboard.png", "left01.jpg", "left02.jpg"]:
    ...
    print("No checkerboard sample found. Using synthetic pattern.")
    print("Run: python curriculum/sample_data/download_samples.py")
    return create_checkerboard(rows=6, cols=8), False


# Load or create checkerboard
checkerboard, is_real_image = load_checkerboard_image()

# Board size depends on whether it's real or synthetic
if is_real_image:
    # Standard OpenCV sample checkerboards are 9x6 inner corners
    board_size = (9, 6)
else:
    board_size = (7, 5)  # Inner corners (cols-1, rows-1)

print(f"Checkerboard size: {checkerboard.shape}")
print(f"Inner corners to detect: {board_size}")
...
def load_grid_image():
    """Load a real image for undistortion demo or create grid."""
    # Try to load a real image
    for sample in ["building.jpg", "sudoku.png", "lena.jpg"]:
        img = get_image(sample)
        if img is not None:
            print(f"Using sample image for undistortion demo: {sample}")
            return cv2.resize(img, (400, 400))

    # Fallback: Create a grid
    print("Using synthetic grid for undistortion demo.")
...
def load_perspective_image():
    """Load image for perspective transform demo."""
    # Try to load a real image (sudoku is great for perspective demo)
    for sample in ["sudoku.png", "building.jpg", "road.jpg"]:
        img = get_image(sample)
        if img is not None:
            print(f"Using sample image for perspective demo: {sample}")
            return cv2.resize(img, (400, 400))

    # Fallback: Create test image with quadrilateral
    print("Using synthetic image for perspective demo.")
    img = np.zeros((400, 400, 3), dtype=np.uint8)
    pts = np.array([[80, 80], [320, 100], [350, 350], [50, 300]], np.int32)
    cv2.fillPoly(img, [pts], (0, 255, 0))
    cv2.putText(img, "Skewed", (120, 200), cv2.FONT_HERSHEY_SIMPLEX,
                1, (255, 255, 255), 2)
    return img


test_img = load_perspective_image()

# Define source and destination points
# (These are example points - adjust based on actual image content)
